...
model = YOLO('yolov8s.pt')  # Ensure GPU is used for inference
...
class VideoProcessingWorker(QRunnable):
    def __init__(self, urls, start_time, alerts_callback):
        ...
        self.grabbers = [FrameGrabber(url) for url in urls]

    def run(self):
        try:
            while not self.stop_flag:
                frames = [grabber.read() for grabber in self.grabbers]
                ...
                grid_size = int(np.ceil(np.sqrt(len(valid_frames))))
                grid_frame = np.zeros((height * grid_size, width * grid_size, 3), dtype=np.uint8)

                processed_frames = [
                    process_frame(frame, self.start_time, self.alerts_callback) for frame in valid_frames
                ]
...
class DetectionApp(QMainWindow):
    def __init__(self):
        ...
        self.timer.setInterval(1000 // frame_rate)
    ...
    def process_videos(self):
        ...
        self.worker = VideoProcessingWorker(self.current_urls, self.start_time, self.add_alert)
