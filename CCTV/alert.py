...
object_last_positions = defaultdict(tuple)  # Stores the last position of each object
object_stationary_since = defaultdict(float)  # Stores the timestamp when the object became stationary
    ...
    for det in detections:
        ...
        if last_position == (center_x, center_y):
            ...
            if label not in object_stationary_since:
                object_stationary_since[label] = current_time  # Start stationary timer
            elif current_time - object_stationary_since[label] >= stationary_threshold:
                # Print alert if stationary for 5 seconds
                print(f"ALERT: {label_translation.get(label, label)} has been stationary for {stationary_threshold} seconds!")
        else:
            # Object moved
    ...
    def run(self):
        while not self.stop_flag:
            try:
                ...
                # Get frame rate and dimensions from the first camera
